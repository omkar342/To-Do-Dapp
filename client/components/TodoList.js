...
const TodoList = ({addTask, setTaskInput, tasks, deleteTask}) => <div className='w-[70%] bg-[#354ea3] py-4 px-9 rounded-[30px] overflow-y-scroll'>
  <Navbar />
  ...
  <ul>
    ...
    {tasks.map((task)=>{
      return (
        <Task key={task.id} taskText={task.taskText} deleteTask={deleteTask(task.id)} />
      )
    })}
  </ul>
</div>
